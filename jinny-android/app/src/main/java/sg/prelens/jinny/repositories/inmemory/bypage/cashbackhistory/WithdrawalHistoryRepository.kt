...
class WithdrawalHistoryRepository(private val api: ApiLink,
    ...
    override fun fetchHistory(pageSize: Int): Listing<CashBackHistory> {
        val sourceFactory = CashBackDashboardHistoryDataSourceFactory(api, networkExecutor, MyCashBackPageFragment.TYPE_WITHDRAWAL)
        ...
        val refreshState = Transformations.switchMap(sourceFactory.sourceWithdrawalLiveData) {
            it.initialLoad
        }
        return Listing(
                pagedList = livePagedList,
                networkState = Transformations.switchMap(sourceFactory.sourceWithdrawalLiveData, {
                    it.networkState
                }),
                retry = {
                    sourceFactory.sourceWithdrawalLiveData.value?.retryAllFailed()
                },
                refresh = {
                    sourceFactory.sourceWithdrawalLiveData.value?.invalidate()
                },
